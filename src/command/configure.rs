...
/// Command to handle interactive configuration of Luminatti features.
...
impl ConfigureCommand {
    ...
    /// 4. Saves the configuration to `~/.config/luminatti/luminatti.config.json`
    pub fn execute() -> Result<(), LuminattiError> {
        println!("\n  \x1b[1;36mLuminatti Configuration\x1b[0m\n");
        ...
        println!(
            ...
            config_path.join("luminatti.config.json").display()
        );
    ...
    fn select_provider() -> Result<&'static ProviderInfo, LuminattiError> {
        ...
        let selection = Select::new("Select your default AI provider:", options)
            ...
            .prompt()
            .map_err(|e| LuminattiError::ConfigurationError(e.to_string()))?;
    ...
    /// is local (e.g. Ollama).
    fn get_api_key(provider: &ProviderInfo) -> Result<Option<String>, LuminattiError> {
    ...
    fn get_model_name(provider: &ProviderInfo) -> Result<Option<String>, LuminattiError> {
        let prompt = format!(
    ...
    /// Resolves the path to the configuration directory (`~/.config/luminatti`).
    fn get_config_path() -> Result<std::path::PathBuf, LuminattiError> {
        let mut path = home_dir().ok_or_else(|| {
            LuminattiError::ConfigurationError("Could not determine home directory".to_string())
        })?;
        path.push(".config");
        path.push("luminatti");
        Ok(path)
    ...
    fn save_config(
        ...
        let config_file = config_dir.join("luminatti.config.json");
