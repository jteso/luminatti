...
use crate::{error::LuminattiError, provider::LuminattiProvider};
...
impl OperateCommand {
    pub async fn execute(&self, provider: &LuminattiProvider) -> Result<(), LuminattiError> {
        LuminattiCommand::print_with_mdcat(format!("`query`: {}", &self.query))?;
        ...
        let operate_result = extract_operate_response(&result)
            .map_err(|e| LuminattiError::CommandError(e.to_string()))?;
