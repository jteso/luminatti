...
pub fn get_new_content(filename: &str, refs: &DiffRefs, backend: &dyn VcsBackend) -> String {
    match refs {
        ...
        DiffRefs::WorkingTree | DiffRefs::RangeToWorkingTree { .. } => {
            backend
                .get_working_copy_file_content(Path::new(filename))
                .unwrap_or_default()
        }
    }
}

fn file_diff(filename: String, refs: &DiffRefs, backend: &dyn VcsBackend) -> FileDiff {
...
/// Load one known changed file without scanning every changed path first.
///
/// Sidebar selections already originate from the changed-file list, so this
/// avoids re-enumerating the repository before rendering the selected file.
pub fn load_file_diff(
    ...
    let refs = DiffRefs::from_options(options, backend);
    file_diff(filename, &refs, backend)
}
...
pub fn load_file_diffs(options: &DiffOptions, backend: &dyn VcsBackend) -> Vec<FileDiff> {
    ...
    get_changed_files(options, backend)
        .into_iter()
        .map(|filename| file_diff(filename, &refs, backend))
        .collect()
}
...
mod tests {
    ...
    #[test]
    fn test_load_file_diff_reads_the_backend_working_copy_not_process_cwd() {
        let dir = make_temp_dir("git-diff-backend-workdir");
        git(&dir, &["init"]);
        git(&dir, &["config", "user.email", "test@example.com"]);
        git(&dir, &["config", "user.name", "Test User"]);
        fs::write(dir.join("worktree-only-fixture.txt"), "before\n").expect("write initial");
        git(&dir, &["add", "."]);
        git(&dir, &["commit", "-m", "initial"]);
        fs::write(dir.join("worktree-only-fixture.txt"), "after\n").expect("write change");

        // Deliberately leave the process cwd alone. Desktop repositories can
        // be opened from a different directory than the app process.
        let backend = GitBackend::new(&dir).expect("should open repo");
        let options = super::super::DiffOptions {
            reference: None,
            pr: None,
            detect_pr: false,
            file: Some(vec!["worktree-only-fixture.txt".to_string()]),
            watch: false,
            theme: None,
            stacked: false,
            focus: None,
        ...
        let diff = load_file_diff(
            "worktree-only-fixture.txt".to_string(),
            &options,
            &backend,
        );
