...
/// captured (e.g. when an agent shim does `output=$(luminatti diff)`),
...
fn max_h_scroll(state: &mut AppState, term_width: u16) -> u16 {
    ...
    state.ensure_cache();
    let (_, _, max_old, max_new) = state.line_metrics_ref();
...
fn run_app_internal(
    ...
    'main: loop {
        ...
        if state.file_diffs.is_empty() {
            ...
            let (file_added, file_removed, _, _) = state.line_metrics_ref();
                    ...
                    match key.code {
                        ...
                        KeyCode::Enter => {
                            ...
                            if state.search_state.has_query() {
                                ...
                                {
                                    state.scroll = line.saturating_sub(5);
                                }
                            }
                        }
                        ...
                        if let Some(result) = modal.handle_input(key, term_height) {
                            match result {
                                ...
                                ModalResult::JumpToLine {
                                    ...
                                    state.scroll = sbs_line_index.min(max_scroll);
                                    active_modal = None;
                                }
                        ...
                        MouseEventKind::ScrollDown | MouseEventKind::ScrollUp => {
                            ...
                            if in_sidebar {
                                ...
                                if scroll_delta > 0 {
                                    ...
                                    state.scroll =
                                        state.scroll.saturating_sub((-scroll_delta) as usize);
                                }
                            }
                        }
                ...
                Event::Key(key) if key.kind == KeyEventKind::Press && active_modal.is_none() => {
                    ...
                    match key.code {
                        ...
                        KeyCode::Char('d') if key.modifiers.contains(KeyModifiers::CONTROL) => {
                            let half_screen = visible_height / 2;
                            state.scroll = state.scroll
                                .saturating_add(half_screen)
                                .min(max_scroll);
                        }
                        ...
                        KeyCode::PageDown => {
                            state.scroll = state.scroll.saturating_add(20).min(max_scroll);
                        }
                        KeyCode::PageUp => {
                        ...
                        KeyCode::Char('{') => {
                            if !state.file_diffs.is_empty() {
                                ...
                                let prev_hunk = if state.focused_hunk.is_none() {
                                    hunks
                                        .iter()
                                        .rposition(|&h| h < state.scroll.saturating_sub(5))
                        ...
                        KeyCode::Char('s') => {
                            if !state.annotations.is_empty() {
                                ...
                                let msg = format!(
                                    "Exit luminatti and write {} {} to stdout?\n\n\
