...
fn syntax_class(index: usize) -> Option<SyntaxClass> {
    match HIGHLIGHT_NAMES.get(index).copied()? {
        "comment" => Some(SyntaxClass::Comment),
        "keyword" => Some(SyntaxClass::Keyword),
        "string" | "string.special" => Some(SyntaxClass::String),
        "number" => Some(SyntaxClass::Number),
        "function" | "function.builtin" | "function.method" | "function.macro" => {
            Some(SyntaxClass::Function)
        }
        "type" | "type.builtin" | "constructor" => Some(SyntaxClass::Type),
        "property" | "variable.member" => Some(SyntaxClass::Property),
        "constant" | "constant.builtin" => Some(SyntaxClass::Constant),
        "operator" => Some(SyntaxClass::Operator),
        "punctuation" | "punctuation.bracket" | "punctuation.delimiter" => {
            Some(SyntaxClass::Punctuation)
        }
        "tag" => Some(SyntaxClass::Tag),
        "attribute" => Some(SyntaxClass::Attribute),
        "variable" | "variable.builtin" | "variable.parameter" => Some(SyntaxClass::Variable),
        _ => None,
    }
}

/// Produce renderer-neutral, byte-indexed syntax spans for each source line.
/// Keeping this independent from Ratatui lets the native GPUI workspace share
/// the exact same Tree-sitter grammar and highlight queries as the terminal UI.
pub fn syntax_ranges_by_line(content: &str, filename: &str) -> HashMap<usize, Vec<SyntaxRange>> {
    let mut lines = HashMap::<usize, Vec<SyntaxRange>>::new();
    let Some(lang_config) = get_config_for_file(filename) else {
        return lines;
    };
    let mut highlighter = Highlighter::new();
    let Ok(events) = highlighter.highlight(&lang_config.config, content.as_bytes(), None, |_| None)
    else {
        return lines;
    };
    ...
    for event in events.flatten() {
        match event {
            HighlightEvent::Source { start, end } => {
                let class = current_highlight.and_then(syntax_class);
                for part in content[start..end].split_inclusive('\n') {
                    let visible = part.strip_suffix('\n').unwrap_or(part);
                    if let Some(class) = class {
                        if !visible.is_empty() {
                            lines.entry(line_number).or_default().push(SyntaxRange {
                                range: column..column + visible.len(),
                                class,
                            });
                        }
                    }
            ...
            HighlightEvent::HighlightStart(highlight) => current_highlight = Some(highlight.0),
...
function hello(): string {
    ...
    #[test]
    fn test_renderer_neutral_typescript_ranges_are_line_relative() {
        let ranges =
            syntax_ranges_by_line("const answer: number = 42;\nreturn answer;\n", "example.ts");
        assert!(ranges.get(&1).is_some_and(|line| {
            line.iter().any(|span| span.class == SyntaxClass::Keyword)
                && line.iter().all(|span| span.range.end <= 26)
        }));
        assert!(ranges
            .get(&2)
            .is_some_and(|line| { line.iter().any(|span| span.class == SyntaxClass::Keyword) }));
    }
