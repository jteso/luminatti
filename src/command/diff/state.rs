...
pub struct AppState {
    ...
    /// Added/removed counts and maximum old/new line widths for the cached diff.
    cached_line_metrics: Option<(usize, usize, usize, usize)>,
...
fn compute_total_line_stats(file_diffs: &[FileDiff], skip_file: Option<usize>) -> (usize, usize) {
    let mut added = 0usize;
    let mut removed = 0usize;
    for (index, diff) in file_diffs.iter().enumerate() {
        if diff.is_binary || skip_file == Some(index) {
            continue;
        }
...
impl AppState {
    pub fn new(file_diffs: Vec<FileDiff>, focus_file: Option<&str>) -> Self {
        ...
        let (total_added, total_removed) =
            compute_total_line_stats(&file_diffs, Some(current_file));
        let mut state = Self {
            file_diffs,
            sidebar_items,
            sidebar_visible,
        ...
        if !state.file_diffs.is_empty() {
            state.ensure_cache();
            if !state.file_diffs[state.current_file].is_binary {
                let (added, removed, _, _) = state.line_metrics_ref();
                state.total_added += added;
                state.total_removed += removed;
            }
            state.scroll = state
                .hunks_ref()
                .first()
                .map(|&h| h.saturating_sub(5))
                .unwrap_or(0);
            state.focused_hunk = (!state.hunks_ref().is_empty()).then_some(0);
        }
        state
    }
    ...
    pub fn ensure_cache(&mut self) {
        ...
        if needs_recompute {
            ...
            if diff.is_binary {
                self.cached_side_by_side = Some((current, Vec::new()));
                self.cached_hunks = Some((current, Vec::new()));
                self.cached_total_lines = Some((current, 0));
                self.cached_line_metrics = Some((0, 0, 0, 0));
                return;
            }
            ...
            let mut metrics = (0usize, 0usize, 0usize, 0usize);
            for line in &sbs {
                match line.change_type {
                    crate::command::diff::types::ChangeType::Insert => metrics.0 += 1,
                    crate::command::diff::types::ChangeType::Delete => metrics.1 += 1,
                ...
                if let Some((_, text)) = &line.old_line {
                    metrics.2 = metrics.2.max(text.chars().count());
                }
                if let Some((_, text)) = &line.new_line {
                    metrics.3 = metrics.3.max(text.chars().count());
                }
            }
            let total = sbs.len();
            ...
            self.cached_line_metrics = Some(metrics);
    ...
    pub fn line_metrics_ref(&self) -> (usize, usize, usize, usize) {
        self.cached_line_metrics.unwrap_or_default()
    }
    ...
    pub fn get_highlighters(&mut self) -> (&FileHighlighter, &FileHighlighter) {
        ...
        if needs_recompute {
            let diff = &self.file_diffs[current];
            let (old_hl, new_hl) = if diff.is_binary {
                (FileHighlighter::default(), FileHighlighter::default())
            } else {
                (
                    FileHighlighter::new(&diff.old_content, &diff.filename),
                    FileHighlighter::new(&diff.new_content, &diff.filename),
                )
            };
    ...
    pub fn reload(&mut self, file_diffs: Vec<FileDiff>, changed_files: Option<&HashSet<String>>) {
        ...
        let (total_added, total_removed) = compute_total_line_stats(&self.file_diffs, None);
        ...
        if !self.file_diffs.is_empty() {
            ...
            self.scroll = old_scroll.min(max_scroll);
...
pub fn adjust_scroll_to_line(
    ...
    let new_scroll = if line < scroll_usize + margin {
        ...
        line.saturating_sub(content_height.saturating_sub(margin).saturating_sub(1))
    } else {
        scroll
    };
...
pub fn adjust_scroll_for_hunk(
    ...
    if hunk_line < scroll_usize + top_margin {
        return hunk_line.saturating_sub(top_margin).min(max_scroll);
    }
