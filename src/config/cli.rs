...
#[derive(Parser)]
#[command(name = "luminatti")]
#[command(about = "AI-powered CLI tool for git commit summaries", long_about = None)]
#[command(version)]
pub struct Cli {
    /// Path to configuration file eg: ./path/to/luminatti.config.json
    #[arg(long)]
...
pub enum Commands {
    ...
    /// Launch the native macOS diff reviewer (requires the `desktop` feature)
    Desktop {
        /// Commit reference: SHA, HEAD, HEAD~3..HEAD, main..feature, main...feature
        #[arg(value_parser = clap::value_parser!(CommitReference))]
        reference: Option<CommitReference>,

        /// Initially focus this file
        #[arg(long)]
        focus: Option<String>,
    },
    /// Interactively configure Luminatti (provider, API key)
    Configure,
}
...
mod tests {
    ...
    fn test_vcs_git_parses() {
        let cli = Cli::try_parse_from(["luminatti", "--vcs", "git", "diff"]).unwrap();
        assert_eq!(cli.vcs, Some(VcsOverride::Git));
    }

    #[test]
    fn test_vcs_jj_parses() {
        let cli = Cli::try_parse_from(["luminatti", "--vcs", "jj", "diff"]).unwrap();
        assert_eq!(cli.vcs, Some(VcsOverride::Jj));
    }

    #[test]
    fn test_vcs_not_specified() {
        let cli = Cli::try_parse_from(["luminatti", "diff"]).unwrap();
        assert_eq!(cli.vcs, None);
    }

    #[test]
    fn test_diff_wrap_flag_parses() {
        let cli = Cli::try_parse_from(["luminatti", "diff", "--wrap"]).unwrap();
        match cli.command {
